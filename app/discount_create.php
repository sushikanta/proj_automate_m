...
$tax = 0;
	...
	if($disc_type =='1'){ $total_after_disc = $total_after_tax - ($total_after_tax * $disc_value * 0.01); $disc_pre = $disc_value.'%';}
	if($disc_type =='2'){ $total_after_disc = $total_after_tax - $disc_value; $disc_pre = 'Rs.'.$disc_value;;} //2=amount
...
if($disc_type =='1' || $disc_type =='2'){
	...
	// start- no record on discount_tbl
	if ($empty =='Y'){
	$disc_sl_x = date("jmy").getCounter($con, 12);
 ...
 ******************************/
 ...
 $remark_audit1 = 'Discount Added:'.$disc_pre.' (Registration #: '.$receipt_no.')';
 mysqli_query($con, "INSERT INTO audit_tbl(A_id, A_book, A_action, A_p_id, A_remark, A_user, A_date) VALUES ('$A_id', '26', '22', '$receipt_no', '".$remark_audit1."', '$user_id', NOW())");
  if(mysqli_affected_rows($con) <= 0) { $flag = false; echo "Error: audit_tbl " .mysqli_error($con). ".";}
  else updateCounter($con, 42);
	}

	// start - Yes record on discount_tbl
	if ($empty =='N'){
 ...
 ******************************/
 $A_id = date("jmy").getCounter($con, 42);
 $remark_audit2 = 'Edited Discount:'.$disc_pre.' (Registration #: '.$receipt_no.')';
 mysqli_query($con, "INSERT INTO audit_tbl(A_id, A_book, A_action, A_p_id, A_remark, A_user, A_date) VALUES ('$A_id', '26', '23', '$receipt_no', '".$remark_audit2."', '$user_id', NOW())");
  if(mysqli_affected_rows($con) <= 0) { $flag = false; echo "Error: audit_tbl " .mysqli_error($con). ".";}
  else updateCounter($con, 42);
  }
 ...
 if($disc_type =='N'){
	...
	if($flag){
			mysqli_commit($con);
			header("location:discount_today.php");
			//echo 'done';
			} else {
			mysqli_rollback($con);
 ...
 <form class="form-horizontal inv-form" role="form" method="get" action="#" id="myform">
  ...
  <?php
	...
	if (mysqli_num_rows($check)==0){
			...
			$dsc_value = "";
 			$dsc_remark = "";
			$dsc_code_sl = date("jmy").getCounter($con, 12);
			$empty='Y';
		 }
	 else{
