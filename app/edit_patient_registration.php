...
$pass_receipt_no = $_GET['receipt_no'];
 ...
 $patient_info = mysqli_query($con, "SELECT r.pr_receipt_no, r.pr_patient_id, r.pr_source_id, r.pr_referred_id, r.pr_date, r.pr_dr_prescription, p.PI_name,  p.PI_age_y,  p.PI_age_m,  p.PI_age_d,  p.PI_gender,  p.PI_marital_id,  p.PI_address, p.PI_state_id,  p.PI_district_id,  p.PI_pin,  p.PI_phone, y.PP_tax, y.PP_disc,  y.PP_total, y.PP_net, y.PP_paid, y.PP_bal, y.PP_date, s.state_name, d.district_name FROM patient_registration r LEFT JOIN patient_info p ON p.PI_id = r.pr_patient_id LEFT JOIN patient_payment y ON y.PP_receipt_no = r.pr_receipt_no LEFT JOIN state_tbl s ON s.state_id = p.PI_state_id LEFT JOIN district_tbl d ON d.district_id = p.PI_district_id WHERE r.pr_receipt_no = '$pass_receipt_no'");
  ...
  while($row = mysqli_fetch_array($patient_info))
  {
	  ...
	  $marital_id = $row['PI_marital_id'];
	  $pr_patient_address = $row['PI_address'];
  	  $pr_patient_state_id = $row['PI_state_id'];
	  $pr_patient_district_id = $row['PI_district_id'];
...
<div class="form-group" style="padding-bottom:30px;">    
    ...
    <label for="src_type" class="col-lg-1-4 control-label"> Source </label>
    <div class="col-lg-2">					      
  ...
  <input type="hidden" id="hidden_source_id" name="hidden_source_id" value="<?php echo $pr_source_id; ?>">
    ...
    <label for="dr_letter" class="col-lg-1-3 control-label"> Dr. Ref ?</label>
    ...
    <input type="radio" name="dr_letter" class="form-control" id="dr_letter_no" value ="N" <?php if($dr_id == "NO"){echo "checked";} ?>>No
      ...
      <div class="col-lg-2-1" >
      <input type="text" class="form-control" name="dr_name" placeholder="Search" id="dr_name" value="<?php if($dr_id=="NO"){echo "";}else{echo showDoctor_name($con, $dr_id);}?>">
...
<div class="form-group">   
      ...
      <input type="hidden" name="hidden_receipt_no" value="<?php echo $pass_receipt_no;?>">
  ...
  <input  type="radio" class="form-control" name="patient_sex" class="" id="optionsRadios1" value="M" <?php if($pr_patient_gender == "M"){echo "checked";} ?>>M
  </label>
  <label for="optionsRadios2" class="radio-inline col-lg-1" style="padding-left:30px;">
  <input type="radio" class="form-control" name="patient_sex" id="optionsRadios2" value="F" <?php if($pr_patient_gender == "F"){echo "checked";} ?>>F
  </label>
  </div>
  </div>
  ...
  <input type="text" class="form-control capital" id="state" placeholder="Search State" name="state" value="<?php echo $state_name;?>" maxlength="20">
  ...
  <input type="text" class="form-control capital" id="district" placeholder="Search district" name="district" value="<?php echo $district_name;?>">
...
<div class="form-group">  
  ...
  <div class="col-lg-2-5">
  <textarea class="form-control textBox_height" name="patient_history" id="patient_history" placeholder="Optional (Max.200 Characters)" maxlength="200"></textarea>
  </div> 
  </div> 
  
    <div class="form-control-group">
    <label for="lab_note" class="col-lg-1-3 control-label">Lab Note</label>
    <div class="col-lg-2-1">
    <textarea class="form-control textBox_height" name="lab_note" placeholder="Optional (Max.100 Chars)" id="lab_note" maxlength="100"></textarea>
    </div> 
    </div> 
    
     <div class="form-control-group">
        <label for="prev_history" class="col-lg-1-3 control-label">Old History</label>
        <div class=" col-lg-2-1">
        <textarea class="form-control textBox_height" readonly><?php echo showHistory_patient($con, $pr_patient_id);?></textarea>

        </div> 
        </div>
    
   <div class="form-control-group">
        <label for="prev_history" class="col-lg-1-3 control-label">Old Lab Note</label>
         <div class=" col-lg-2-1">
        <textarea class="form-control textBox_height" readonly><?php echo showLab_note($con, $pr_receipt_no);?></textarea>
        </div> 
        </div>
 ...
 <div class="form-group">
	<div class="form-control-group">
      ...
      <div class="col-lg-2-5">					      
          <label class=" control-label radio-inline col-lg-1" style="margin-left:30px;">
           	<input  type="radio" name="opt_date" class="form-control radio-inline" value ="Y">Yes
          </label>
          <label class="control-label radio-inline col-lg-1" style="padding-left:35px;">
      ...
      <div class="form-control-group">          
      <label for="date_edit" class="col-lg-1-3 control-label">Date</label>
      <div class="col-lg-2-1">
      <input type="text" class="form-control" value="<?php echo date("d-m-Y, h:i:s a", strtotime($pr_date));?>" name="date_edit" id="date_edit" readonly>
       ...
       <div class="form-control-group">
      <label for="inputID" class="col-lg-1-3 control-label">EID</label>
      <div class="col-lg-2-1">
      <input type="text" class="form-control" value="<?php echo 'ED/'.$pass_receipt_no;?>" name="ed_receipt_no" readonly> 
      </div>
      </div>  
...
<div class="panel panel-success">  
 ...
 <div class="panel-body">
   <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered display" id="test_table">
        <thead align="left">
        <tr>
          <th>SL</th>
          <th> Investigation</th>
