...
<body id="myBody">
<?php 
if(isset($_GET['start']) && $_GET['start'] !='' && isset($_GET['stop']) && $_GET['stop'] !='' && isset($_GET['source_id']) && $_GET['source_id'] !=''){
	$_start = mysqli_real_escape_string($con, $_GET['start']);
	$_stop = mysqli_real_escape_string($con, $_GET['stop']);
	$source_id = mysqli_real_escape_string($con, $_GET['source_id']);
	
	$start = date("Y-m-d", strtotime($_start));
	$stop = date("Y-m-d", strtotime($_stop));
	$src_name = showSourceName($con, $source_id);
?>
...
<div class="container">
 <div class="page-content">		    
   ...
   <div class="panel panel-success">  
    ...
    Source <span class="panel-subTitle">( Collection Report - All "<?php echo $src_name; ?>" )</span>
...
<span style="text-decoration: underline; font-weight:bold; font-size:11pt;" class="blue_color">Collection Report ( All " <?php echo $src_name; ?> " )</span> <br/>
...
$result1 = mysqli_query($con, "SELECT y.PP_total, y.PP_tax, y.PP_disc, y.PP_net, y.PP_paid, y.PP_bal, y.PP_date, r.pr_dr_prescription, r.pr_receipt_no, r.pr_source_id, r.pr_referred_id, r.pr_date, p.PI_name FROM patient_registration r LEFT JOIN patient_payment y ON r.pr_receipt_no = y.PP_receipt_no LEFT JOIN patient_info p ON p.PI_id = r.pr_patient_id WHERE DATE_FORMAT(r.pr_date, '%Y-%m-%d') BETWEEN '".$start."' AND '".$stop."' AND r.pr_source_id = '$source_id' ORDER BY r.pr_referred_id ASC");

if (mysqli_num_rows($result1)!= 0){ 
?>  
<table class="table-hover" style="width:100%; margin-top:20px;">
<thead align="left">
...
while ($row = mysqli_fetch_array($result1))
{
  ...
  ?>
 	<tr>
       ...
       <td><?php $ref_name = showReferral($con, $pr_source_id, $pr_referred_id); echo $ref_name; ?> </td>
