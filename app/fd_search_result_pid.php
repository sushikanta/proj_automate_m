 ...
 <div class="inv-main" id="printableArea">	
  ...
  <div class="panel panel-success">  <!----------------------START Patient list-------------->
	<div class="panel-heading light_purple_color">
     <h3 class="panel-title"><i class="fa fa-windows fa-spin fa-lg fa-fw"></i> Search Result 
...
if($tid == '3'){ // NAME
$result = mysqli_query($con, "SELECT r.pr_receipt_no, r.pr_date, r.pr_status_id, r.pr_patient_id, r.pr_source_id, p.PI_name, p.PI_marital_id, p.PI_gender, p.PI_age_y, p.PI_age_m, p.PI_age_d, p.PI_address, p.PI_state_id, p.PI_district_id, p.PI_pin, p.PI_phone, patient_payment.PP_bal, s.state_name, d.district_name, m.marital_name, g.gender_name FROM patient_registration r LEFT JOIN patient_payment ON patient_payment.PP_receipt_no = r.pr_receipt_no LEFT JOIN patient_info p ON p.PI_id = r.pr_patient_id  LEFT JOIN state_tbl s ON s.state_id = p.PI_state_id LEFT JOIN district_tbl d ON d.district_id = p.PI_district_id LEFT JOIN marital_tbl m ON m.marital_id = p.PI_marital_id LEFT JOIN gender_tbl g ON g.gender_id = p.PI_gender WHERE r.pr_patient_id='$patient_id'");
}

if($tid == '4'){ // DATE
	$start = $_GET['start'];
    $end = $_GET['end'];
$result = mysqli_query($con, "SELECT r.pr_receipt_no, r.pr_date, r.pr_status_id, r.pr_patient_id, r.pr_source_id, p.PI_name, p.PI_marital_id, p.PI_gender, p.PI_age_y, p.PI_age_m, p.PI_age_d, p.PI_address, p.PI_state_id, p.PI_district_id, p.PI_pin, p.PI_phone, patient_payment.PP_bal, s.state_name, d.district_name, m.marital_name, g.gender_name FROM patient_registration r LEFT JOIN patient_payment ON patient_payment.PP_receipt_no = r.pr_receipt_no LEFT JOIN patient_info p ON p.PI_id = r.pr_patient_id  LEFT JOIN state_tbl s ON s.state_id = p.PI_state_id LEFT JOIN district_tbl d ON d.district_id = p.PI_district_id LEFT JOIN marital_tbl m ON m.marital_id = p.PI_marital_id LEFT JOIN gender_tbl g ON g.gender_id = p.PI_gender WHERE DATE_FORMAT(r.pr_date,'%Y-%m-%d') BETWEEN '".$start."' AND '".$end."' ORDER BY r.pr_date DESC");
}
$sl_no=1;
...
?>
 <tr id="<?php echo $receipt_no; ?>">
      ...
      <button class="btn-info btn_count text-right" style="background-color:#4A8CF7;" type="submit">
...
$(document).ready(function() {
  ...
  $('.form_receipt').submit(function() {	
    myWindow1 = window.open('', 'patient_receipt.php', "width=1200,height=550,resizeable,scrollbars");
