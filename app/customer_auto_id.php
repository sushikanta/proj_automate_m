<?php require_once("config.php");?>
 <?php  
	...
	$rs = mysqli_query($con, 'SELECT p.PI_id, p.PI_name, p.PI_gender, p.PI_age_y, p.PI_age_m, p.PI_age_d, p.PI_address, p.PI_phone, m.marital_name FROM patient_info p LEFT JOIN marital_tbl m ON m.marital_id = p.PI_marital_id WHERE p.PI_id LIKE "%'.mysqli_real_escape_string($con, $_REQUEST['term']).'%"');
	$data = array();
