<?php require_once("config.php");?>
 <?php
	...
	$rs = mysqli_query($con, 'SELECT r.pr_receipt_no, r.pr_patient_id, r.pr_date, p.PI_name, p.PI_age_y, p.PI_age_m, p.PI_age_d, p.PI_address, m.marital_name, g.gender_name FROM patient_registration r LEFT JOIN patient_info p ON p.PI_id = r.pr_patient_id LEFT JOIN marital_tbl m ON m.marital_id = p.PI_marital_id LEFT JOIN gender_tbl g ON g.gender_id = p.PI_gender WHERE r.pr_receipt_no LIKE "'.mysqli_real_escape_string($con, $_REQUEST['term']).'%" ORDER BY r.pr_date DESC');
	$data = array();
		if (mysqli_num_rows($rs) == 0){
		    $data[] = array(
					'label' => "No Registration Found, please recheck",
					'value' => "",
					'receipt_no' => "",
				);
			}
