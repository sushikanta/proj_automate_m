...
if(isset($_GET['receipt_no']))
{
 ...
 $patient_info = mysqli_query($con, "SELECT `patient_registration`.*, `patient_info`.*, `marital_tbl`.`marital_name`, `state_tbl`.`state_name`, `district_tbl`.`district_name`, `pin_tbl`.`pin_code` FROM `patient_registration` LEFT JOIN `patient_info` ON `patient_info`.`PI_id` = `patient_registration`.`pr_patient_id` LEFT JOIN `marital_tbl` ON `marital_tbl`.`marital_id` = `patient_info`.`PI_marital_id` LEFT JOIN `state_tbl` ON `state_tbl`.`state_id` = `patient_info`.`PI_state_id` LEFT JOIN `district_tbl` ON `district_tbl`.`district_id` = `patient_info`.`PI_district_id` LEFT JOIN `pin_tbl` ON `pin_tbl`.`pin_id` = `patient_info`.`PI_pin_id` WHERE `patient_registration`.`pr_receipt_no` = '".$receipt_no."'");
 
  while($p_info = mysqli_fetch_array($patient_info))
  {
	  
  	  $pr_patient_name = $p_info['PI_name'];
	  $pr_patient_age = $p_info['PI_age'];
	  $pr_patient_age_ext = $p_info['PI_age_ext'];
...
<div class="row">
<label class="col-lg-2 control-label text-right">Registration ID :</label> <div class="col-lg-3"><?php echo $receipt_no;?></div> 
...
<div class="row">
<label for="inputName" class="col-lg-2 control-label text-right">Name :</label> <div class="col-lg-3"><?php echo $pr_patient_name;?></div> 
<label for="inputName" class="col-lg-1-5 control-label text-right"><i class="fa fa-male"></i><i class="fa fa-female"></i> :</label> <div class="col-lg-2">
<?php $age_ext = showAge_ext($con, $pr_patient_age_ext); if($pr_patient_gender =='1'){echo "Male, ".$pr_patient_age." ".$age_ext;} elseif($pr_patient_gender =='2'){echo "Female, ".$pr_patient_age." ".$age_ext;} ?></div>
<label for="inputName" class="col-lg-1-5 control-label text-right">Marital Status :</label> <div class="col-lg-2"><?php echo $pr_marital;?></div>
</div>

<div class="row">

  <label for="inputName" class="col-lg-2 control-label text-right">Address :</label>
  <div class="col-lg-6"><?php echo $pr_patient_address.', '.$pr_patient_district; if($pr_patient_pin =="0"){ echo ', '.$pr_patient_state;}else{echo ', '.$pr_patient_state." - ".$pr_patient_pin;}?></div> 
...
<div class="col-lg-1-6"> +91 <?php echo $pr_patient_phone;?></div>
   <!--<label for="inputName" class="col-lg-1-5 control-label text-right">State :</label>
  <div class="col-lg-2"><?php if($pr_patient_pin ==""){ echo $pr_patient_state;}else{$pr_patient_state." - ".$pr_patient_pin;}?></div>-->
</div>
