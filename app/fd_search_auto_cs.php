<?php require_once("config.php");?>
 <?php  
	...
	$rs = mysqli_query($con, 'SELECT p.PI_id, p.PI_name, p.PI_age_y, p.PI_age_m, p.PI_age_d, p.PI_address, m.marital_name, g.gender_name FROM patient_info p LEFT JOIN marital_tbl m ON m.marital_id = p.PI_marital_id LEFT JOIN gender_tbl g ON g.gender_id = p.PI_gender WHERE p.PI_id LIKE "'.mysqli_real_escape_string($con, $_REQUEST['term']).'%"');
	$data = array();
		...
		$data[] = array(
			...
			while( $row = mysqli_fetch_array($rs)){				
				...
				'value' => $row['PI_id'].", ".$row['PI_name'].", ".$row['PI_address'].' ( '.$row['gender_name']." / ".$age.", ".$row['marital_name'].' )',
