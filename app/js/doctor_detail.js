$(document).ready(function() {
	 ...
	 $('#state').autocomplete({             // START Autocomplete for state-doctor
	...
	$("#district").autocomplete({    // START Autocomplete for District
	...
	$("#pin_code").autocomplete({    // START Autocomplete for pin_code
        source: function(request, response) {
            $.ajax({
		...
		focus: function( event, ui ) {
            $( "#pin_code").val( ui.item.label );
            return false;
            },
		select: function (event, ui) {
			$('#hidden_pin').val(ui.item.pin_id);
			return false;
		    }
	...
	$('#new_clinic_state').autocomplete({             // START Autocomplete for state-clinic
		source:'autocomplete_state.php', 
		minLength:1,
		focus: function( event, ui ) {
            $( "#new_clinic_state" ).val( ui.item.label );
            return false;
            },
		select: function (event, ui) {
			$('#hidden_new_clinic_state').val(ui.item.state_id);
			return false;
		    }
		});
		  
		$("#district_clinic").autocomplete({    // START Autocomplete for District - clinic
        source: function(request, response) {
            $.ajax({
                url: "autocomplete_district.php",
                dataType: "json",
		...
		focus: function( event, ui ) {
            $( "#district_clinic").val( ui.item.label );
            return false;
            },
		select: function (event, ui) {
			$('#hidden_district_clinic').val(ui.item.district_id);
			return false;
		    }
    });   // END Autocomplete for District
		
	$("#pin_clinic").autocomplete({    // START Autocomplete for pin_code - clinic
        source: function(request, response) {
            $.ajax({
                url: "autocomplete_pin.php",
                dataType: "json",
                data: {
                    term: request.term,
                    district: $("#hidden_district_clinic").val(),                    
                },
                success: function(data) {
                    response(data);
                }
		...
		focus: function( event, ui ) {
            $( "#pin_clinic").val( ui.item.label );
            return false;
            },
		select: function (event, ui) {
			$('#hidden_pin_clinic').val(ui.item.pin_id);
			return false;
		    }
    });   // END Autocomplete for patient_pin	  
